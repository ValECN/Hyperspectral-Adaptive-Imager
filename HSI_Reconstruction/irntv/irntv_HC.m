function [U, itstat] = irntv_nv(S, KC, lambda, pars)
% HC modified to account for different input-output space dimension
%  
...
for w = 1:w
    s1 = s1(:,:,w);
    s2 = s2(:,:,w);
    body = (s1'*s2)/norm(s1) * norm(s2);
    SAM = SAM + norm(acos(body));
end
SAM = (2*SAM/(pi*W))
return
